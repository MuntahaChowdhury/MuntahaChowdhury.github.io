...
import { CaretCircleLeft, CaretCircleRight, MinusCircle, XCircle } from "@phosphor-icons/react";
...
export default function ImageGallery({ images }: ImageGalleryProps) {
    ...
    const [v_images, setImages] = useState(images);
    ...
    useEffect(() => {   //for image sliding
        ...
        const interval = setInterval(() => {
            setSelectedImage((prev) => {
                const currentIndex = v_images.findIndex((img) => img === prev);
                const nextIndex = (currentIndex + 1) % v_images.length;
                return v_images[nextIndex];
            });
    ...
    const handleImageArrowClick = (dir: string) => {
        setSelectedImage((prev) => {
            ...
            if (dir === "prev") {
                index = (currentIndex - 1 + v_images.length) % v_images.length;
            } else if (dir === "next") {
                index = (currentIndex + 1) % v_images.length;
            }

            return v_images[index];
        });
    };
    ...
    const handleRemoveImage = (index: number, from: 'slider' | 'dltform') => {

        if (from === 'slider') {
            const v_imgUrlToDlt = v_images[index];
            ...
            const v_updatedSkuImages = [...v_images];
        ...
        else if (from === 'dltform') {
            const v_imgUrlToDlt = v_formDataToDlt.skuImages[index];

            setFormDataDlt((prevFormData) => ({
                ...prevFormData,
                skuImages: prevFormData.skuImages.filter((url) => url !== v_imgUrlToDlt)
            }));

            // Step 2: Add the image to `setImages`
            setImages((prevImages) => {
                // Make a copy of the current `setImages` array
                const updatedSkuImages = [...prevImages];

                // Add the image to the array (it seems like you want to add it back)
                updatedSkuImages.push(v_imgUrlToDlt);

                // Return the updated array
                return updatedSkuImages;
            });
        }
    ...
    const handleDelete = async () => {
        if (v_formDataToDlt.skuImages.length === 0) return;

        for (const v_imgBeingDeleted of v_formDataToDlt.skuImages) {

            const v_payload = JSON.stringify({
                fileKey: v_imgBeingDeleted,
            });
            ...
            try {
                ...
                alert(`Image deleted successfully! FileKey: ${v_imgBeingDeleted}`);
            } catch (error: unknown) {
                if (error instanceof Error) {
                    ...
                    alert(`Deletion failed: ${error || JSON.stringify(error)}`);
                }
            }
        }
            ...
            {v_images && v_images.length !== 0 ? (
                ...
                <div className="flex gap-4">
                    ...
                    <div>
                        ...
                        >
                            {v_images.map((img, index) => (
                                <SwiperSlide key={index}>
                                    <div className="relative">
                                        <img
                                            src={img}
                                            alt="Thumbnail"
                                            className={`w-14 h-14 border object-cover rounded cursor-alias hover:scale-110 duration-300 ${selectedImage === img ? "border-2 border-blue-500" : "border border-bru2"}`}
                                            onClick={() => setSelectedImage(img)}
                                        />
                                        <MinusCircle size={18} color="red" weight="duotone" className="absolute top-0 right-0 bg-black rounded-2xl cursor-pointer" onClick={() => handleRemoveImage(index, 'slider')} />
                                    </div>
                    ...
                    <div className="flex items-center relative shadow-md">
                        <img src={selectedImage} alt="Selected" className="w-[60vh] h-[60vh] object-cover rounded-lg shadow-lg cursor-zoom-in" onClick={() => setIsImageFullOpen(true)} />
                    ...
                    {v_formDataToDlt && (
                        <div>
                            <h2 className="text-2xl font-semibold text-bru5 mb-2">Removed images</h2>
                            <div className="flex gap-4">
                                {v_formDataToDlt.skuImages.map((img, index) => (
                                    <div key={index} className="relative">
                                        ...
                                        />
                                        <XCircle size={18} color="green" weight="duotone" className="absolute top-0 right-0 bg-black rounded-2xl cursor-pointer" onClick={() => handleRemoveImage(index, 'dltform')} />
                                    </div>
                                ))}
                            </div>
                            <button onClick={handleDelete} className="bg-red-700 border-red-900 hover:bg-red-600 hover:border-red-900 w-full mt-4 text-sm">
                                Confirm Delete
                            </button>
